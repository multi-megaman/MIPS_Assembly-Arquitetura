...
mesa_format: #Params (None)
    la $t0, mesas #$t0 comeï¿½a no inicio do gerenciador de mesas e vai percorrendo de mesa em mesa
    lhu $t1, limite_mesas #$t1 define a quantidade mï¿½xima de mesas (15)
    lbu $t2, tamanho_codigo_mesa 
    lhu $t3, tamanho_mesa
    li $t4, 1 #codigo de cada mesa, comeï¿½a em 1 e vai atï¿½ $t1 (limite_mesas)
	...
	#add $t0, $t0, $t5 #indo para o registro de pedidos
        ...
        clear_register_de_pedidos:
            ...
            sub $t7, $t7, 1
            bgtu $t7, $0, clear_register_de_pedidos #Caso $t7 ainda nï¿½o seja igual a zero, significa que ainda temos bytes para zerar 
	#Quando $t0 sair desse loop, ele vai estar apontando para a primeira posiï¿½ï¿½o da proxima mesa (seu respectivo codigo)
        j loop_mesas_format
...
checar_ocupacao_mesa: #Params ($a0 -> id da mesa que serah checada | int)
    	...
    	lhu $t1, limite_mesas #$t1 define a quantidade mï¿½xima de mesas (15)
   	addi $t2, $0, 1 #$t2 vai percorrer de 1 atï¿½ o limite
    	lhu $t3, tamanho_mesa
	...
	ble $a0, $t1, dentro_do_limite    # Se $a0 <= limite_cardapio(20) significa que ele estï¿½ dentro do limite 1-20
	j falha_mesa_codigo_alcance  # Pula para o final da funï¿½ï¿½o
